...
def modifyServerPorts(nodeName, serverName, serverPortNode):
    if serverPortNode.getAttributeNode("host"):
        AdminTask.modifyServerPort(serverName,
            ['-nodeName', nodeName, '-endPointName', serverPortNode.attributes["endPointName"].value, '-host',
             serverPortNode.attributes["host"].value, '-port', serverPortNode.attributes["port"].value, '-modifyShared',
             'true'])
    else:
        AdminTask.modifyServerPort(serverName,
            ['-nodeName', nodeName, '-endPointName', serverPortNode.attributes["endPointName"].value, '-port',
             serverPortNode.attributes["port"].value, '-modifyShared', 'true'])
    ...
    for dataSourceNode in getChildElements(jdbcProviderNode, "DataSource"):
        ...
        if not len(dataSource):
            ...
            for resourcePropertyNode in getChildElements(dataSourceNode, "ResourceProperties/J2EEResourceProperty"):
                ...
                % (scopePath, jdbcProviderParams['name'], dataSourceProperties['name']))
                resourceProperty = AdminConfig.getid(
                    '%sJDBCProvider:%s/DataSource:%s/J2EEResourcePropertySet:/J2EEResourceProperty:%s/'
                    % (scopePath, jdbcProviderParams['name'], dataSourceProperties['name'],
...
def createMailSessions(scopePath, mailProviderNode):
    ...
    for mailSessionNode in getChildElements(mailProviderNode, "MailSession"):
        ...
        if len(resourcePropertiesNodeList) > 0:
            ...
            if not len(resourcePropertySet):
            ...
            for resourcePropertyNode in resourcePropertiesNodeList:
                resourceProperty = AdminConfig.getid(
                    '%sMailProvider:/MailSession:%s/J2EEResourcePropertySet:/J2EEResourceProperty:%s/'
...
def updateVariableMap(scopePath, scope, variableMapNode):
    ...
    for variableEntryNode in getChildElements(variableMapNode, "VariableSubstitutionEntry"):
        ...
        if not existing:
            ...
            AdminConfig.create('VariableSubstitutionEntry', variableMap,
                [['symbolicName', symbolicName], ['value', value]])
