...
def StartApplication( applicationModel, nodeName, serverName ):
    log(INFO_,
        "StartApplication: applicationModel=" + applicationModel.name + " nodeName=" + nodeName + " serverName=" + serverName + " ...")
    appMgrID = WebSphere.AdminControl.queryNames(
        "type=ApplicationManager,node=" + nodeName + ",process=" + serverName + ",*")
    length = len(appMgrID)
    ...
    if length >= 1:
        ...
        if temp > 0:
            log(WARNING_,
                "StartApplication: Exception trying to start " + applicationModel.name + " " + nodeName + " " + serverName)
            return
        else:
            if len(started) > 0:
                log(INFO_, started)
...
def StopApplication( applicationModel, nodeName, serverName ):
    log(INFO_,
        "StopApplication: applicationModel=" + applicationModel.name + " nodeName=" + nodeName + " serverName=" + serverName + " ...")
    appMgrID = WebSphere.AdminControl.queryNames(
        "type=ApplicationManager,node=" + nodeName + ",process=" + serverName + ",*")
    length = len(appMgrID)
    ...
    if length >= 1:
        ...
        if temp > 0:
            log(WARNING_,
                "StopApplication: Exception trying to stop " + applicationModel.name + " " + nodeName + " " + serverName)
        else:
            if len(stopped) > 0:
...
def checkApplicationRunning( nodeName, serverName, applicationModel ):
    ...
    try:
        _excp_ = 0
        appID = WebSphere.AdminControl.completeObjectName(
            "type=Application,node=" + nodeName + ",Server=" + serverName + ",name=" + applicationModel.name + ",*")
    except:
        _type_, _value_, _tbck_ = sys.exc_info()
        _excp_ = 1
        #endTry
    ...
    if temp > 0:
        log(WARNING_,
            "checkApplicationRunning: Exception trying to getID for " + applicationModel.name + " " + nodeName + " " + serverName)
        #endIf
    length = len(appID)
    ...
    while ( retries < 20 )  and ( length == 0 ):
        retries = retries + 1
        log(INFO_,
            "checkApplicationRunning: not yet started, " + applicationModel.name + " " + nodeName + " " + serverName + ", retries=" + `retries` + " ...")
        try:
            _excp_ = 0
            sleepDelay(10)
            appID = WebSphere.AdminControl.completeObjectName(
    ...
    if retries > 0:
        log(INFO_,
            "checkApplicationRunning: " + nodeName + " " + serverName + " " + applicationModel.name + " had slow start, status retries=" + `retries`)
        #endIf
    ...
    if length > 0:
        ...
        log(ERROR_,
            "checkApplicationRunning: " + applicationModel.name + " " + nodeName + " " + serverName + " DID NOT START.")
        #endElse
