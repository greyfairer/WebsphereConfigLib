...
def syncRippleStart (action, applicationModels):
    ...
    for clusterName in Globals.clusters.keys():
        ...
        if len(clusterTarget.inactiveServers) == 0:
...
def stopSyncStart (action, nodeName, containedServerNames):
        ...
        if (action == "update"):
                ...
                #endFor
        if (action == "update"):
                log(VERBOSE_, "stopSyncStart: FUTURE: plugin-cfg.xml RE-ACTIVATE node=" + nodeName + " servers=" + `containedServerNames`)
        #endIf
...
def syncNDNode (nodeName):
        ...
        log(VERBOSE_, "syncNDNode: FUTURE: check for nodeSync EAR expansion complete (for now just delay " + `EarExpandDelay` + " secs)")
...
def checkClustersStarted(clusters):
    ...
    for clusterName in clusters.keys():
        if len(clusters[clusterName].inactiveServers) == 0:
            ...
            while len(clusterId) == 0 and retryConnect < 5:
                    try:
                            ...
                            _type_, _value_, _tbck_ = sys.exc_info()
                            log(WARNING_, "checkClusterStarted WebSphere.AdminControl exception=" + `_type_` + " clusterName=" + clusterName)
                    if len(clusterId) == 0:
...
def checkServerStopped (nodeName, serverName):
        ...
        log(VERBOSE_, "checkServerStopped: " + nodeName + " " + serverName + " actualState=" + actualState + " desiredState=" + desiredState)
